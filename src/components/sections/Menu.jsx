  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Yahan Change Kiya Hai: grid-cols-2 (Mobile) aur gap-3 (Mobile) */}
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-8">
          {coffeeProducts.map((product, index) => (
            <Link
              key={product.id}
              to={`/product/${product.id}`}
              className="group relative flex flex-col overflow-hidden rounded-2xl sm:rounded-3xl bg-white dark:bg-surface-dark border border-border-light dark:border-white/5 transition-all hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/5 sm:hover:-translate-y-2 duration-300 shadow-sm dark:shadow-none"
            >
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                className="flex flex-col h-full"
              >
                {/* Image Container: padding mobile par kam ki hai */}
                <div className="aspect-square w-full overflow-hidden bg-accent-light dark:bg-accent-dark/30 p-4 sm:p-8 flex items-center justify-center relative">
                  <div className="absolute inset-0 bg-radial-gradient from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <img
                    src={product.image}
                    alt={product.name}
                ...
                {/* Content Section: p-3 (Mobile) aur p-6 (Desktop) */}
                <div className="flex flex-1 flex-col p-3 sm:p-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start mb-2 gap-1">
                    <h3 className="text-sm sm:text-xl font-bold text-text-main dark:text-white group-hover:text-primary transition-colors duration-300 font-display line-clamp-1">
                      {product.name}
                    </h3>
                    <span className="font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-lg text-[10px] sm:text-sm">
                      {product.price}
                    </span>
                  </div>

                  {/* Description: Mobile par chhupa di hai ya line-clamp lagaya hai space bachane ke liye */}
                  <p className="mt-1 text-[10px] sm:text-sm text-text-muted line-clamp-1 sm:line-clamp-2 leading-relaxed">
                    {product.description}
                  </p>

                  {/* Bottom Row: Mobile par button ko stylish icon banaya hai */}
                  <div className="mt-auto pt-3 sm:pt-6 flex items-center justify-between border-t border-border-light dark:border-white/5">
                    <span className="hidden sm:inline-block text-xs font-bold text-text-muted uppercase tracking-wider bg-background-light dark:bg-background-dark/50 px-2 py-1 rounded">
                      {product.tag}
                    </span>

                    <div className="flex items-center justify-between w-full sm:w-auto gap-2">
                      <span className="hidden sm:flex items-center gap-1 text-text-muted text-sm font-bold">
                      ...
                      <button
                        ...
                        className="flex flex-1 sm:flex-none items-center justify-center gap-2 rounded-full bg-surface-dark text-white dark:bg-white px-3 py-2 sm:px-4 sm:py-2 text-[10px] sm:text-sm font-bold dark:text-background-dark transition-all hover:bg-primary hover:text-white active:scale-95 duration-200"
                      >
